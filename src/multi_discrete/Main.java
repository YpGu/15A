/**
	...
	Main.java: implement the unified model with multiple discrete distribution (doc online: /ypgu/doc/)
	...
	-- The problem is every PAIR of link is associated with a latent parameter (posterior \gamma) 
	-- the background mixture is much better than the idp component, which makes \gamma to be nearly 0 
	-- as a result, \pi will be (1,0), which is not what we want  
**/
...
public class Main
{
	...
	public static double[][] gamma;					// N * N
	...
	init(String[] args) {
		...
		pi = new double[2]; gamma = new double[N][N];
		...
		if (USE_BKG && USE_IPM) {				// both
			...
			for (int i = 0; i < N; i++) 
				for (int j = 0; j < N; j++) 
					gamma[i][j] = 0.4 + 0.2 * rand.nextDouble();
		}
